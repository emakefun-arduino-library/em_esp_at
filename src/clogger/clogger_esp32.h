...
class Clogger {
 public:
  ...
  static void Log(const int32_t severity, const char *file_name, const uint32_t line_num, const char *function, const char *fmt, ...) {
    auto now = std::chrono::system_clock::now();
    auto time_sec = std::chrono::system_clock::to_time_t(now);
    auto time_since_epoch = now.time_since_epoch();
    auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(time_since_epoch % std::chrono::seconds(1)).count();
    ...
    const auto header_length =
        snprintf(nullptr, 0, header_fmt, tm.tm_hour, tm.tm_min, tm.tm_sec, ms, SeverityToChar(severity), file_name, line_num, function);
    ...
    snprintf(buffer.data(), total_size, header_fmt, tm.tm_hour, tm.tm_min, tm.tm_sec, ms, SeverityToChar(severity), file_name, line_num, function);
...
#define CLOGD(fmt, ...) CLOGI(fmt, ##__VA_ARGS__)
